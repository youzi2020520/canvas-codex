...
本プロジェクトは **Elastic License 2.0 (ELv2)** で提供されています（[`LICENSE`](LICENSE) 参照）。

ソースコードは公開されており、閲覧・学習・個人利用・コントリビューションが可能です。**無断での**商用利用、ホスティングサービスとしての提供、名称変更や著作権表示の削除を伴う再配布は**禁止**されています。商用ライセンスや協業については作者までお問い合わせください。

商標・ブランド名・ロゴは作者に帰属し、本ライセンスはそれらの使用権を付与しません。
