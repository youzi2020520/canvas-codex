...
function openGenerationDialog(objectIds = [], anchorBounds = null) {
  ...
  dialog.innerHTML = `
    <section class="generation-dialog" role="dialog" aria-modal="true" aria-labelledby="generationDialogTitle">
      <header>
        <div>
          <strong id="generationDialogTitle">${escapeHtml(tr("generationDialogTitle"))}</strong>
          <span>${escapeHtml(tr("generationDialogHint"))}</span>
        </div>
      ...
      <button type="button" class="generation-dialog-confirm" data-generation-confirm>${escapeHtml(tr("createGenerator"))}</button>
    </section>`;
  const initialMode = references.length ? "image" : "text";
  ...
  dialog.addEventListener("click", (event) => {
    ...
    if (event.target.closest("[data-generation-confirm]")) {
      const mode = dialog.dataset.mode;
      closeGenerationDialog();
      if (mode === "image" && references.length) createStudioFromReferences(references);
