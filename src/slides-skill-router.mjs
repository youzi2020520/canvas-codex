...
export function selectSlideSkills(action, outline = []) {
  ...
  if (routes.has("echarts") || routes.has("data-viz") || /\b(data|comparison)\b|图表|数据|趋势|分布|占比|对比|矩阵|散点|柱状|折线|漏斗/.test(text)) selected.push("create-slide-chart");
  if (routes.has("svg-diagram") || /\b(process|roadmap|solution)\b|流程|架构|关系|路径|时间线|步骤|因果|系统图|示意图/.test(text)) selected.push("create-slide-diagram");
  if (routes.has("art-direction-imagegen") || routes.has("image-crop-focal-point") || /\b(cover|section|case-study)\b|主视觉|照片|图片|插画|场景|人物|产品/.test(text)) selected.push("generate-slide-visual");
